...
class Users {
    ...
    public static function readAll() {
        global $db;
        $sql = "SELECT u.*,r.role_name FROM users u ,roles r where u.role_id = r.id";
        $res = $db->query($sql);
        if ($res) {
    ...
    public function update($id) {
        global $db;
        $sql = "UPDATE users SET id='{$this->id}', name='{$this->name}', email='{$this->email}', password='{$this->password}', role_id='{$this->role_id}', created_at='{$this->created_at}', photo='{$this->photo}', phone='{$this->phone}' WHERE id = $id";
        if ($db->query($sql)) {
